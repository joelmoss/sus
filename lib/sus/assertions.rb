...
module Sus
  class Assertions
    ...
    def initialize(identity: nil, target: nil, output: Output.buffered, inverted: false, orientation: true,
                   isolated: false, distinct: false, measure: false, verbose: false)
    ...
    attr_reader :identity, :target, :output, :level, :verbose, :clock, :skipped, :errored
    ...
    def print(output, verbose: @verbose)
      ...
      if @count.zero?
        ...
        output.write(:passed, @passed.size, ' passed', :reset, ' ') if @passed.any?

        output.write(:failed, @failed.size, ' failed', :reset, ' ') if @failed.any?

        output.write(:deferred, @deferred.size, ' deferred', :reset, ' ') if @deferred.any?

        output.write(:skipped, @skipped.size, ' skipped', :reset, ' ') if @skipped.any?

        output.write(:errored, @errored.size, ' errored', :reset, ' ') if @errored.any?

        output.write('out of ', total, ' total (', @count, ' assertions)')
      end
    end
    ...
    class Assert
      ...
      attr_reader :identity, :backtrace, :assertions
    ...
    def assert(condition, message = nil)
      ...
      if condition
        ...
        @output.assert(condition, @orientation, message || 'assertion passed', backtrace) if !@orientation || @verbose
      else
        @failed << assert

        @output.assert(condition, @orientation, message || 'assertion failed', backtrace) if @orientation || @verbose
      end
    end
    ...
    def each_failure(&block)
      ...
      return yield(self) if failed? and @distinct
    ...
    def error!(error)
      ...
      @output.error(error, @identity, @verbose)
    end
    ...
    def nested(target, identity: nil, isolated: false, distinct: false, inverted: false, **options)
      ...
      output = if isolated or distinct
                 @output.buffered
               else
                 @output
               end
      ...
      assertions = self.class.new(identity: identity, target: target, output: output, isolated: isolated,
                                  inverted: inverted, orientation: orientation, distinct: distinct, verbose: @verbose, **options)
    ...
    def resolve_into(parent)
      ...
      resolve! if @isolated and deferred?
